...
class Usersmodel {
  ...
  static async createUser(nombres, apellidos, foto, perfil, usuario, contraseña) {
    let pool;
    try {
  ...
  static async updateUser(id_usuario, nombres, apellidos, foto, perfil, usuario) {
    let pool;
    try {
      // Conectar a la base de datos PostgreSQL
      pool = await connectToPostgres();
      ...
      const query = `
      UPDATE usuario
      SET nombres = $1, apellidos = $2, foto = $3, perfil = $4, usuario = $5
      WHERE id_usuario = $6
  `;


      // Ejecutar la consulta con parámetros
      await pool.query(query, [nombres, apellidos, foto, perfil, usuario, id_usuario]);
