...
class Usersmodel {
  ...
  static async createUser(nombres,apellidos,ci,dirrecion,telefono,fecha_naci) {
    ...
    try {
      ...
      // Consulta para insertar una nuevo miembro en la base de datos
      const query = `
      ...
      console.error("Error al crear la miembro:", error);
      return false;
    } finally {
  ...
  static async updateUser(id_miembro, nombres,apellidos,ci,dirrecion,telefono, fecha_naci) {
    let pool;
    try {
      // Conectar a la base de datos PostgreSQL
      pool = await connectToPostgres();
      ...
      const query = `
        UPDATE miembro
        SET nombres = $1, apellidos = $2, ci = $3, dirrecion = $4, telefono = $5, fecha_naci = $6
        WHERE id_miembro = $7
      `;


      // Ejecutar la consulta con parámetros
      await pool.query(query, [nombres,apellidos,ci,dirrecion,telefono, fecha_naci, id_miembro]);

      console.log("Miembro actualizado correctamente");
      return true;
    } catch (error) {
      console.error("Error al actualizar la miembro:", error);
      return false;
    } finally {
      // Desconectar de la base de datos
      if (pool) {
  ...
  // Método para cambiar el estado de la miembro
  ...
  static async deleteUser(userId) {
    try {
      ...
      console.error("Error al eliminar la miembro:", error);
      return false;
    }
  }
}
